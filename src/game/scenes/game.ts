...
export class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.escapeTheFud = this.sound.add(KEYS.ESCAPETHEFUD, {
            loop: true,
            seek: 118
        })
        ...
        this.player = this.physics.add.sprite(400, 525, this.selectedGotchi?.spritesheetKey || '');
        this.player.setSize(43, 50);
        this.player.setCollideWorldBounds(true);
        this.player.setScale(1);

        //@ts-ignore
        //this.gotchi.setTraits(50,50,50,50)  // setting gotchi traits
        //@ts-ignore
    ...
    update() 
    {
        ...
        this._shipKeyboardHandler(this.player);
    ...
    private _enemyFires() 
    {
        ...
        {
            //@ts-ignore
            if (livingSushi.sprite === KEYS.SUSHI1)
            {
                //@ts-ignore
                livingSushi.shoot(eB[0], this.player, this.suBullSpeed)

            }//@ts-ignore
            else if (livingSushi.sprite === KEYS.SUSHI2)
            {
                //@ts-ignore
                livingSushi.shoot(eB[0], eB[1], this.player, this.suBullAngle2, this.suBullSpeed)
                                
            }//@ts-ignore
            else if (livingSushi.sprite === KEYS.SUSHI3)
            {
                //@ts-ignore
                livingSushi.shoot(eB[0], eB[1],  eB[2], this.player, this.suBullAngle3, this.suBullSpeed)
    ...
    private _fireBullet() {
        ...
        if (this.time.now > this.bulletTime) {
            ...
            if (bullet) {
                bullet.shoot(this.player.x, this.player.y - 18, this.gBulletSpeed);
